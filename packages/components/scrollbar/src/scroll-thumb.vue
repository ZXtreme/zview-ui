<template>
  <div ref="scrollbar_bar" :class="[
    ...
    <div ref="scrollbar_thumb" class="z-scrollbar__thumb" :style="scrollbarThumb_style"></div>
  </div>
</template>
...
import { ScrollThumbProps } from "./scroll-thumb"
const props = defineProps(ScrollThumbProps);
...
const setScrollTop: SetScrollTop = (top: number) => {
  ...
  // height.value * props.retio! 滚动条可滚动的范围
  // sHeight 超出可视区的范围(scrollTop的最大取值)
  // top scrollTop的当前取值
  // val 滚动条当前的top值
  // 存在关系：val / (height.value * props.retio) = top / sHeight
...
// 根据滚动条的top值设置view视图的滚动位置scrollTop
...
// thumb移动
const scrollbar_thumb = ref<null | HTMLDivElement>(null);
