    ...
    <div class="example">
      ...
      <div class="op-btns">
        <ElTooltip content="在 Github 中编辑" :show-arrow="false">
          <z-icon name="github" @click="toGithub"></z-icon>
        </ElTooltip>
        <ElTooltip content="复制代码" :show-arrow="false">
          <z-icon name="fuzhi" @click="copyCode"></z-icon>
        </ElTooltip>
        <ElTooltip content="查看源代码" :show-arrow="false">
          <z-icon name="code" @click="toggleSourceVisible()"></z-icon>
        </ElTooltip>
      </div>
      ...
      <Transition name="el-fade-in-linear">
        <div v-show="sourceVisible" class="example-float-control" @click="toggleSourceVisible(false)">
          <z-icon name="ArrowTopBold" @click="copyCode"></z-icon>
          <span>隐藏源码</span>
        </div>
      </Transition>
    </div>
...
const toGithub = () => {
  ...
  window.open(
    `https://github.com/ZXtreme/zview-ui/tree/main/packages/components/${name}/src/${name}.vue`,
    "_blank",
  );
};
